...
@property (nonatomic, weak) NSDictionary *authTokens;
...
- (void)loginWithFacebookID:(NSString *)fbToken
{
    ...
    [self sendPostRequestForEndpoint:kOAuthEndpoint withParameters:paramDict completion:^(id responseItem, NSURLResponse *response, NSError *error) {
        if (!error) {
            ...
            //NSString *expiresIn = [responseItem objectForKey:@"expires_in"];
            
            [self.manager registerKey:AccessToken forIdentifier:kAccessTokenIdentifier];
